...
const CommentListComponent = ({
  ...
  const cancelComment = (e, cno) => {
    ...
    textarea.value = commentStates.find(
      (comment) => comment.id === cno
    ).content;
  ...
  const cancelReply = (cno) => {
    setReplyStates((prevStates) =>
      prevStates.map((comment) =>
        comment.id === cno
          ? { ...comment, isEditing: !comment.isEditing, content: "" }
          : comment
      )
    );
  };
  ...
  const cancelModiReply = (e, cno) => {
    ...
    textarea.readOnly = true;
    textarea.style.border = "0";
    textarea.value = replyModiStates.find(
      (comment) => comment.id === cno
    ).content;
  ...
  return (
    <>
      {commentList.dtoList ? (
        commentList.dtoList.map((comment, index) => (
          <div id={comment.cno} key={comment.cno}>
            ...
            <div>
              {comment.state > 0 ? (
                <>
                  <img
                    src="../../images/article/deleteUserIcon.svg"
                    alt="profile"
                  />
                  <p>******</p>
                </>
                ...
                <>
                  <img
                    src={`${RootUrl()}/uploads/user/${comment.thumb}`}
                    alt="profile"
                  />
                  <p>{comment.nick}</p>
              ...
              <h3>
                ...
                {comment.updateDate && (
                  <span> {formatRelativeTime(comment.updateDate)} 수정</span>
                )}
              </h3>
              <div>
                {comment.state > 0 ? null : (
                  <button
                    className="heart"
                    onClick={(e) => heartComment(e, comment.cno)}
                  >
                ...
                {comment.state > 0 ? null : (
                  <>
                    {loginSlice.userno === comment.userNo && (
                      <>
                        {commentStates.find(
                          (state) => state.id === comment.cno && state.isEditing
                        ) ? (
                          ...
                          <button
                            onClick={(e) => modifyComment(e, comment.cno)}
                          >
                            <FontAwesomeIcon
                              icon={faPen}
                              color="#1e1e1e"
                    ...
                    {loginSlice.userno === comment.userNo && (
                      <>
                        {commentStates.find(
                          ...
                          <button
                            onClick={(e) => cancelComment(e, comment.cno)}
                          >
                            <FontAwesomeIcon
                              icon={faXmark}
                              color="#1e1e1e"
                          ...
                          <button
                            onClick={(e) => deleteComment(e, comment.cno)}
                          >
                            <FontAwesomeIcon
                              icon={faTrashCan}
                              color="#1e1e1e"
            ...
            <textarea ref={autoResizeTextarea} readOnly>
              {comment.content}
            </textarea>
            ...
            {replyStates.find(
              ...
              <div className="wrtieReplyBox">
                <section>
                  ...
                  />
                  <textarea
                    onChange={(e) => writeReply(e, comment.cno)}
                  ></textarea>
                </section>
                <section>
                  <button onClick={() => completeReply(comment.cno)}>
                    작성
                  </button>
                  ...
                  <div id={reply.cno} key={reply.cno}>
                    <div>
                      ...
                      {reply.state > 0 ? (
                        ...
                        <>
                          <img
                            src={`${RootUrl()}/uploads/user/${reply.thumb}`}
                            alt="profile"
                          />
                          <p>{reply.nick}</p>
                      ...
                      <h3>
                        ...
                        {reply.updateDate && (
                          <span>
                            {" "}
                            {formatRelativeTime(reply.updateDate)} 수정
                          </span>
                        )}
                      </h3>
                      <div>
                        {reply.state > 0 ? null : (
                          <>
                            <button
                              className="heart"
                              onClick={(e) => heartComment(e, reply.cno)}
                            >
                              <FontAwesomeIcon
                                icon={faHeart}
                                color="#FF0000"
                            ...
                            {loginSlice.userno === reply.userNo && (
                              <>
                                {replyModiStates.find(
                                  (state) =>
                                    state.id === reply.cno && state.isEditing
                                ) ? (
                                  <button
                                    onClick={(e) => saveModiReply(e, reply.cno)}
                                  >
                                    <FontAwesomeIcon
                                      icon={faFloppyDisk}
                                      color="#1e1e1e"
                                  ...
                                  <button
                                    onClick={(e) => modifyReply(e, reply.cno)}
                                  >
                                    <FontAwesomeIcon
                                      icon={faPen}
                                      color="#1e1e1e"
                            ...
                            {loginSlice.userno === reply.userNo && (
                              <>
                                {replyModiStates.find(
                                  ...
                                  <button
                                    onClick={(e) => deleteComment(e, reply.cno)}
                                  >
                                    <FontAwesomeIcon
                                      icon={faTrashCan}
                                      color="#1e1e1e"
